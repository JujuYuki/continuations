...
\indent\textbox{\underline{Value-typing} TV-BASVAL TV-NAME\\
	\begin{flalign*}\text{\underline{Process typing }}&\\
		& \text{T-PAR T-SUM}&\\
		& \text{T-MAT T-NIL}&\\
	...
	\begin{flalign*}\text{\underline{Transitions for base-$\pi$}} &\\
		& \text{OUT INP} &\\
		& \text{TAU MAT} &\\
		& \text{SUM-L PAR-L} &\\
		& \text{COMM-L} &\\
...
\indent\textbox{\underline{Subtyping rules}\\
	\begin{flalign*}& \text{SUB-REFL SUB-TRANS} &\\
		& \text{SUB-$\sharp$I SUB-$\sharp$O} &\\
		& \text{SUB-II SUB-OO} &\\
		& \text{SUB-BS} &
	\end{flalign*}
